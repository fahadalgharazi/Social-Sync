// Backend/src/routes/auth.routes.js
import { Router } from 'express';
import { supabaseAnon, supabaseAdmin } from '../config/supabase.js';
import fetch from 'node-fetch';
import ngeohash from 'ngeohash';
import { validate } from '../middlewares/validate.js';
import { signupSchema, loginSchema } from '../validators/auth.validator.js';

const router = Router();

/**
 * LESSON: Thin Routes Pattern
 *
 * Routes should be "thin" - they should only:
 * 1. Define the HTTP method and path
 * 2. Apply middleware (validation, auth, etc.)
 * 3. Call the appropriate service/controller
 * 4. Return the response
 *
 * Business logic should live in SERVICES, not routes.
 * We'll refactor to use services in the next step.
 * For now, we're focusing on validation.
 */

/**
 * Helper: Convert ZIP code to geographic data
 *
 * This is a "pure" function - given the same input, it always returns the same output.
 * Pure functions are easier to test and reason about.
 *
 * @param {string} zip - Valid ZIP code (already validated by schema)
 * @returns {Promise<{lat: number, lng: number, city: string, state: string, geohash: string}>}
 * @throws {Error} If ZIP code cannot be resolved
 */
async function resolveZipCode(zip) {
  const response = await fetch(`https://api.zippopotam.us/us/${zip}`);

  if (!response.ok) {
    throw new Error('ZIP code not found in database');
  }

  const data = await response.json();
  const place = data.places?.[0];

  if (!place) {
    throw new Error('No location data found for ZIP code');
  }

  const lat = Number(place.latitude);
  const lng = Number(place.longitude);

  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    throw new Error('Invalid coordinates for ZIP code');
  }

  return {
    lat,
    lng,
    city: place['place name'],
    state: place['state abbreviation'],
    geohash: ngeohash.encode(lat, lng, 6)
  };
}

/**
 * POST /api/auth/signup
 *
 * NOTICE: validate(signupSchema) runs BEFORE our handler
 * If validation fails, the handler never runs - user gets a 400 error
 * If validation passes, req.body is guaranteed to match our schema
 *
 * This means:
 * - No more manual if (!email) checks
 * - No more manual .trim().toLowerCase()
 * - No more regex validation inline
 * - req.body is CLEAN and SAFE to use
 */
router.post('/signup', validate(signupSchema), async (req, res, next) => {
  try {
    // Destructure validated data - ALL transformations already applied!
    const {
      email,      // Already trimmed and lowercase
      password,   // Already validated for strength
      firstName,  // Already trimmed
      lastName,   // Already trimmed
      zip,        // Already validated format
      gender,     // Optional, can be null
      bio,        // Optional, can be null
      interests   // Already converted to array
    } = req.body;

    // 1. Create auth user in Supabase
    const { data: authData, error: authError } = await supabaseAnon.auth.signUp({
      email,
      password
    });

    if (authError) {
      console.error('[SIGNUP] Supabase auth error:', authError);
      return res.status(400).json({
        success: false,
        message: authError.message || 'Failed to create account'
      });
    }

    const user = authData.user;
    if (!user?.id) {
      return res.status(500).json({
        success: false,
        message: 'Account created but user ID not returned'
      });
    }

    // 2. Resolve ZIP code to geographic data
    let geoData;
    try {
      geoData = await resolveZipCode(zip);
    } catch (geoError) {
      console.error('[SIGNUP] ZIP resolution error:', geoError);
      return res.status(400).json({
        success: false,
        message: geoError.message
      });
    }

    // 3. Create user record in custom users table
    const { error: userInsertError } = await supabaseAdmin
      .from('users')
      .insert([{
        id: user.id,
        email: email,
        created_at: new Date().toISOString()
      }]);

    if (userInsertError) {
      console.error('[SIGNUP] User insert error:', userInsertError);
      return res.status(400).json({
        success: false,
        message: 'Failed to save user record'
      });
    }

    // 4. Create user profile data
    const { error: profileError } = await supabaseAdmin
      .from('user_data')
      .upsert([{
        id: user.id,
        first_name: firstName,
        last_name: lastName,
        gender: gender || null,
        bio: bio || null,
        city: geoData.city,
        state: geoData.state,
        zipcode: zip,
        latitude: geoData.lat,
        longitude: geoData.lng,
        geohash: geoData.geohash,
        interests: interests,
        created_at: new Date().toISOString()
      }], { onConflict: 'id' });

    if (profileError) {
      console.error('[SIGNUP] Profile insert error:', profileError);
      return res.status(400).json({
        success: false,
        message: 'Failed to save user profile'
      });
    }

    // 5. Return success response
    return res.status(201).json({
      success: true,
      message: 'Account created successfully',
      user: {
        id: user.id,
        email: email
      }
    });

  } catch (error) {
    console.error('[SIGNUP] Unexpected error:', error);
    return next(error);
  }
});


/**
 * POST /api/auth/login
 *
 * validate(loginSchema) ensures:
 * - email is valid format, trimmed, lowercase
 * - password is not empty
 */
router.post('/login', validate(loginSchema), async (req, res, next) => {
  try {
    const { email, password } = req.body;

    // Attempt authentication with Supabase
    const { data, error } = await supabaseAnon.auth.signInWithPassword({
      email,
      password
    });

    if (error) {
      return res.status(401).json({
        success: false,
        message: 'Invalid email or password'
      });
    }

    const { access_token, refresh_token, user } = data.session;

    /**
     * LESSON: Secure Cookie Configuration
     *
     * Cookies store session tokens. Security is critical:
     * - httpOnly: JavaScript can't read the cookie (prevents XSS attacks)
     * - secure: Only sent over HTTPS (prevents man-in-the-middle attacks)
     * - sameSite: Controls cross-site request behavior (prevents CSRF attacks)
     * - maxAge: When the cookie expires (forces re-login)
     */
    const isProduction = process.env.NODE_ENV === 'production';
    const cookieOptions = {
      httpOnly: true,
      secure: isProduction,
      sameSite: isProduction ? 'none' : 'lax',
      path: '/',
      maxAge: 3600000 // 1 hour in milliseconds
    };

    // Set authentication cookies
    res.cookie('sb_at', access_token, cookieOptions);
    res.cookie('sb_rt', refresh_token, cookieOptions);

    return res.json({
      success: true,
      message: 'Login successful',
      user: {
        id: user.id,
        email: user.email
      }
    });

  } catch (error) {
    console.error('[LOGIN] Unexpected error:', error);
    return next(error);
  }
});

/**
 * POST /api/auth/logout
 *
 * No validation needed - just clear cookies
 * This is idempotent (safe to call multiple times)
 */
router.post('/logout', (_req, res) => {
  res.clearCookie('sb_at', { path: '/' });
  res.clearCookie('sb_rt', { path: '/' });

  return res.json({
    success: true,
    message: 'Logged out successfully'
  });
});

/**
 * GET /api/auth/me
 *
 * Returns current authenticated user's info
 * Used by frontend to check if user is logged in
 */
router.get('/me', async (req, res) => {
  const token = req.cookies?.sb_at;

  if (!token) {
    return res.status(401).json({
      success: false,
      message: 'Not authenticated'
    });
  }

  const { data, error } = await supabaseAnon.auth.getUser(token);

  if (error || !data?.user) {
    return res.status(401).json({
      success: false,
      message: 'Invalid or expired session'
    });
  }

  return res.json({
    success: true,
    user: {
      id: data.user.id,
      email: data.user.email
    }
  });
});

export default router;
