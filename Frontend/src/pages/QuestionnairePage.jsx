...
export default function QuestionnairePage() {
  ...
  const handleOpenEnded = (val) => setResponses((r) => ({ ...r, openEnded: val }));
        ...
        {currentSection < traitNames.length ? (
            ...
            <CardContent className="space-y-8">
              {QUESTIONS[traitNames[currentSection]].map((question, i) => (
                <div key={i} className="space-y-4">
                  <h3 className="font-medium text-gray-800 leading-relaxed">{question}</h3>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    <div className="flex gap-2 flex-1 justify-center">
                      {[1, 2, 3, 4, 5].map((value) => {
                        const isSelected =
                          responses[`${traitNames[currentSection]}-${i}`] === value;
                        return (
                          <button
                            key={value}
                            type="button"
          ...
          <Card className="bg-white/70 backdrop-blur-xl border-white/50 shadow-xl">
            ...
            <CardContent className="space-y-6">
              ...
              {/* Final Navigation */}
              <div className="flex justify-between items-center pt-6 border-t border-gray-100">
                <Button variant="outline" onClick={prevSection} className="bg-white/50">
                  <ChevronLeft className="w-4 h-4 mr-2" />
                  Previous
                </Button>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {traitNames.map((trait, index) => {
            ...
            return (
              <Card
                key={trait}
                className={`bg-white/50 backdrop-blur-sm border transition-all duration-200 cursor-pointer hover:shadow-lg ${
                  isCurrent
                ...
                <CardContent className="p-4 flex items-center space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {QUESTIONS[trait].filter((_, i) => responses[`${trait}-${i}`]).length}/
                      {QUESTIONS[trait].length} answered
                    </p>
                  </div>
                </CardContent>
              </Card>
